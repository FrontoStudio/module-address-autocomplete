...
define([
    ...
    var initAutocomplete = function () {
        var loaded = false;
        uiRegistry.get('block-summary.block-shipping.address-fieldsets.street.0', function(streetEl) {
            if (loaded == false) {
    ...
    fillInAddress = function () {
        ...
        for (var i = 0; i < place.address_components.length; i++) {
            ...
            if (componentForm[addressType]) {
                ...
                if (addressType == 'subpremise') {
                    ...
                    var thisDomID = uiRegistry.get('block-summary.block-shipping.address-fieldsets.'+ elementId).uid;
                    if ($('#'+thisDomID)) {
                        $('#'+thisDomID).val(value);
        ...
        if (street.length > 0) {
            ...
            var domID = uiRegistry.get('block-summary.block-shipping.address-fieldsets.street').elems()[0].uid;
        ...
        var cityDomID = uiRegistry.get('block-summary.block-shipping.address-fieldsets.city').uid;
        if ($('#'+cityDomID)) {
            $('#'+cityDomID).val(city);
            $('#'+cityDomID).trigger('change');
        }
        if (region != '') {
            if (uiRegistry.get('block-summary.block-shipping.address-fieldsets.region_id')) {
                var regionDomId = uiRegistry.get('block-summary.block-shipping.address-fieldsets.region_id').uid;
                if ($('#'+regionDomId)) {
...
return $.trim($(this).text()) == region; })
            ...
            if (uiRegistry.get('block-summary.block-shipping.address-fieldsets.region_id_input')) {
                var regionDomId = uiRegistry.get('block-summary.block-shipping.address-fieldsets.region_id_input').uid;
                if ($('#'+regionDomId)) {
